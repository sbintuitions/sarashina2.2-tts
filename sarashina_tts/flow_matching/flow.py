# Flow Matching Decoder module.
# Modified from FlashCosyVoice (https://github.com/xingchensong/FlashCosyVoice)
...
from .estimator import CausalConditionalDecoder
...
class CausalConditionalCFM(torch.nn.Module):
    def __init__(self, in_channels=320, cfm_params=CfmParams(), n_spks=1, spk_emb_dim=80, estimator: torch.nn.Module = None):
        ...
        # Pre-generate fixed noise for deterministic inference (matching original CosyVoice2)
        _set_all_random_seed(0)
    ...
    def forward(self, mu, mask, n_timesteps, temperature=1.0, spks=None, cond=None, streaming=False):
        ...
        # Use pre-generated noise expanded for batch, matching original CosyVoice2 behavior
        z = self.rand_noise[:, :, :mu.size(2)].to(mu.device).to(mu.dtype).expand(batch_size, -1, -1) * temperature
